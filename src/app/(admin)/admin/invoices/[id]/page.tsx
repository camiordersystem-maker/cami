...
export default function AdminInvoiceDetailPage() {
  ...
  useEffect(() => {
    fetch("/api/feature-flags")
      ...
      .then((json) => {
        if (json) setEmailFeatureEnabled(Boolean(apiData<Record<string, boolean>>(json)[FEATURE_FLAGS.INVOICE_PDF_EMAIL]));
      })
      .catch(() => {});
  }, []);
  ...
  async function sendInvoiceEmail() {
    ...
    setMessage(
      res.ok
        ? { text: "請求書をメールで送付しました", ok: true }
        : { text: apiErrorMessage(json, "送付に失敗しました"), ok: false }
    );
  }
        ...
        <div className="ml-auto flex items-center gap-2">
          {emailFeatureEnabled && !isViewer && (
            <button
              ...
              disabled={sending}
              className="text-sm text-blue-600 border border-blue-300 bg-white hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
            >
              {sending ? "送信中..." : "会員へメール送付"}
            </button>
          )}
